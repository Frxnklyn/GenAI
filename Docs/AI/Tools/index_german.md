...
**Verwendung.** Behalten Sie üblicherweise die standardmäßige Operationsliste bei und beschränken Sie `allowed_paths` auf die Repositorys, die der Agent untersuchen darf. Um eine weitere Operation freizugeben, fügen Sie ihren vordefinierten Namen zu `allowed_commands` hinzu; `stage` wird auf `git add` abgebildet. Unbekannte Namen werden als Konfigurationsfehler abgelehnt. Shell-Operatoren außerhalb von Argumenten in doppelten Anführungszeichen werden gesperrt, während Git-Suchmuster in Anführungszeichen Metazeichen wie Alternativen (`\|`) und Gruppierungen verwenden dürfen. Variablen- und Befehlssubstitutionen sowie Optionen, die Befehlsausgaben schreiben oder externe Diff- und Pager-Hilfsprogramme aufrufen, bleiben gesperrt.
