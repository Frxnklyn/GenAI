...
/**
 * Status message with a button that opens the widget of a configured PowerUI action.
 *
 * @author Brooklyn Fränzschky
 */
class AiResponseStatusMessageWithAction implements AiResponseStatusMessageInterface
{
	private string $type;
	private string $text;
	...
	/**
	 * Creates a status message from an already configured PowerUI action.
	 */
	public function __construct(
		...
		if (! $action instanceof iShowWidget) {
			throw new InvalidArgumentException('An AI response status message action must implement iShowWidget.');
		}
	...
	/**
	 * Returns the stable DOM and action registration ID.
	 */
...
HTML;
	...
	private function validateOrCreateId(?string $id): string
	{
		...
		if (preg_match('/^[A-Za-z][A-Za-z0-9_-]{0,63}$/', $id) !== 1) {
			throw new InvalidArgumentException('Invalid AI response status message action ID.');
		}
