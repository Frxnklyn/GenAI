...
{
	private const ACTION_REGISTRY_NAMESPACE = 'axenox.GenAI.AiStatusMessageWithWidget.v2';
...
HTML;
	...
	private function createActionFromUxon(UxonObject $actionUxon, ?WidgetInterface $contextWidget): ActionInterface
	{
		...
		if ($widgetUxon instanceof UxonObject) {
			...
			if (! $widgetUxon->hasProperty('id_space')) {
				$widgetUxon->setProperty('id_space', $contextWidget->getIdAutogenerated());
			}
			$actionUxon->setProperty('widget', $widgetUxon);
		}

		$this->registerActionUxon($actionUxon, $contextWidget);
		...
		self::configureRegisteredActionWidget($action, $contextWidget, $this->id);

		return $action;
	}

	/**
	 * Gives an automatically generated action dialog the same stable identity in every request.
	 */
	public static function configureRegisteredActionWidget(
		...
		$widget->setId($registrationId . '_widget');
		$widget->setIdSpace($contextWidget->getIdAutogenerated());
	}

	/**
	 * Stores a bounded set of server-generated actions for reconstruction during later AJAX requests.
	 */
	private function registerActionUxon(UxonObject $actionUxon, WidgetInterface $contextWidget): void
	{
		$scope = $contextWidget->getWorkbench()->getContext()->getScopeSession();
		$key = self::getRegistryKey($contextWidget);
		$registry = $scope->getVariable($key, self::ACTION_REGISTRY_NAMESPACE);
		$registry = is_array($registry) ? $registry : [];
		$registry[$this->id] = $actionUxon->toArray();

		if (count($registry) > self::ACTION_REGISTRY_LIMIT) {
			$registry = array_slice($registry, -self::ACTION_REGISTRY_LIMIT, null, true);
		}

		$scope->setVariable($key, $registry, self::ACTION_REGISTRY_NAMESPACE);
		// AI chat responses are produced outside the regular workbench task lifecycle, so queued
		// session-scope variables would otherwise not be persisted for the dialog's AJAX requests.
		$scope->saveContexts();
	}

	/**
	 * Returns status-message actions registered for an AIChat widget in the current session.
	 *
	 * @return UxonObject[]
	 */
	public static function getRegisteredActionUxons(WidgetInterface $contextWidget): array
	{
		$scope = $contextWidget->getWorkbench()->getContext()->getScopeSession();
		$registry = $scope->getVariable(
			self::getRegistryKey($contextWidget),
			self::ACTION_REGISTRY_NAMESPACE
		);
		...
		return array_map(
			static fn (array $uxon): UxonObject => new UxonObject($uxon),
			array_filter($registry, 'is_array')
		);
	}

	/**
	 * Builds the session key without exposing action UXON in browser requests.
	 */
	private static function getRegistryKey(WidgetInterface $contextWidget): string
	{
		return hash(
			'sha256',
			$contextWidget->getPage()->getAliasWithNamespace() . '|' . $contextWidget->getId()
		);
	}
	...
	private function renderWidgetFragment(): string
	{
		...
		// This custom trigger renders the widget directly and therefore bypasses the regular action prefill lifecycle.
		if ($this->action instanceof iPrefillWidget
			&& $this->action->getPrefillWithPrefillData()
			&& $this->action->hasPrefillDataPreset()
		) {
			$widget->prefill($this->action->getPrefillDataPreset());
		}
